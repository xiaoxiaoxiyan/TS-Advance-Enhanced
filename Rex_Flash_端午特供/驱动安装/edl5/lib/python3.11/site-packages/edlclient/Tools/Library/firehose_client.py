...
class firehose_client(metaclass=LogBase):
    def __init__(self, arguments, cdc, sahara, loglevel, printer):
        ...
        env_maxpayload = None
        try:
            env_maxpayload = self._vip_env("EDL_MAXPAYLOAD")
        except Exception:
            env_maxpayload = None
        if env_maxpayload is not None:
            self.cfg.MaxPayloadSizeToTargetInBytes = getint(env_maxpayload)
            try:
                self.arguments["--maxpayload"] = env_maxpayload
            except Exception:
                pass
    ...
    def _vip_env(self, key):
        value = os.environ.get(key)
        if value in (None, "", "None"):
            return None
        return value

    def _vip_arg(self, key, env_key):
        value = None
        try:
            ...
            value = None
        if value in (None, "", "None"):
            value = self._vip_env(env_key)
        return value

    def _vip_enabled(self):
        return self._vip_arg("--vip-digest", "EDL_VIP_DIGEST") is not None

    def _vip_send_xml(self, path, label):
        if path in (None, "", "None"):
            return True
        if not os.path.exists(path):
            self.error("VIP %s file not found: %s" % (label, path))
            return False
        with open(path, "rb") as f:
            data = f.read()
        resp = self.firehose.xmlsend(data)
        if not resp.resp:
            self.error("VIP %s failed: %s" % (label, resp.error))
            return False
        return True

    def _vip_auth(self):
        digest = self._vip_arg("--vip-digest", "EDL_VIP_DIGEST")
        sig = self._vip_arg("--vip-sig", "EDL_VIP_SIG")
        transfer = self._vip_arg("--vip-transfer", "EDL_VIP_TRANSFER")
        verify = self._vip_arg("--vip-verify", "EDL_VIP_VERIFY")
        sha = self._vip_arg("--vip-sha", "EDL_VIP_SHA")
        cfg = self._vip_arg("--vip-cfg", "EDL_VIP_CFG")
        if digest is None and sig is None and transfer is None and verify is None and sha is None and cfg is None:
            return True
        if None in (digest, sig, transfer, verify, sha, cfg):
            self.error("VIP parameters missing, aborting")
            return False
        ...
        self.info("VIP auth: sending digest/transfer/verify/sig/sha/cfg")
        if not self.firehose.send_signed_digest(digest):
            return False
        if not self._vip_send_xml(transfer, "transfercfg"):
            return False
        if not self._vip_send_xml(verify, "verify"):
            return False
        if not self.firehose.send_signed_digest(sig):
            return False
        if not self._vip_send_xml(sha, "sha256init"):
            return False
        if not self._vip_send_xml(cfg, "cfg"):
            return False
        if not self.firehose.supported_functions:
            self.firehose.supported_functions = [
        ...
        if self.cfg.MemoryName == "":
            if "ufs" in self.firehose.supported_functions:
                ...
                self.warning(
                    ...
                    "with \"UFS\",\"NAND\" or \"spinor\" instead !")
                self.cfg.MemoryName = "eMMC"
        if self._vip_enabled():
            if not self._vip_auth():
                return False
            self.connected = True
            return True
            ...
            for partition in partitions:
                if partition == "gpt":
                    ...
                    for lun in luns:
                        ...
                        if self.firehose.cmd_read(lun, 0, 32, partfilename, True, "PrimaryGPT"):
                            self.printer(
                ...
                if res[0]:
                    lun = res[1]
                    rpartition = res[2]
                    if self.firehose.cmd_read(lun, rpartition.sector, rpartition.sectors, partfilename, True, partition):
                        self.printer(
            ...
            for lun in luns:
                ...
                for partitionname in guid_gpt.partentries:
                    ...
                    self.info(
                        ...
                        f"as {filename}.")
                    if self.firehose.cmd_read(lun, partition.sector, partition.sectors, filename, True, partitionname):

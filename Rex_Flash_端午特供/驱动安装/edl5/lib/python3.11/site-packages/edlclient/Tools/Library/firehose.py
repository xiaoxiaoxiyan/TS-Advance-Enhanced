...
class firehose(metaclass=LogBase):
    ...
    def send_signed_digest(self, path):
        if path is None or path == 'None':
            return True
        if not os.path.exists(path):
            self.error("Signed digest not found: " + path)
            return False
        with open(path, "rb") as f:
            data = f.read()
        if len(data) == 0:
            self.error("Signed digest is empty: " + path)
            return False
        self.info("Signed Digest Table (%d bytes) '%s'" % (len(data), path))
        self.cdc.flush()
        self.cdc.xmlread = False
        if not self.cdc.write(data):
            self.error("Failed to send signed digest")
            return False
        rdata = bytearray()
        counter = 0
        timeout = 3
    ...
    def cmd_read(
        ...
        if label is None:
            label = "read"
            if filename_attr:
                label = os.path.splitext(filename_attr)[0] or label
        data = f"<?xml version=\"1.0\" ?><data><read SECTOR_SIZE_IN_BYTES=\"{self.cfg.SECTOR_SIZE_IN_BYTES}\"" + \
               f" num_partition_sectors=\"{num_partition_sectors}\"" + \
               f" physical_partition_number=\"{physical_partition_number}\"" + \
               f" start_sector=\"{start_sector}\"" + \
               f" filename=\"{filename_attr}\" label=\"{label}\"/>\n</data>"
    ...
    def connect(self):
        ...
        signed_digests = None
        try:
            signed_digests = self.args.get("--signeddigests")
        except Exception:
            signed_digests = None
        if signed_digests not in (None, 'None', ''):
            if not self.send_signed_digest(signed_digests):
                return []
        # rsp = self.xmlsend(data, self.skipresponse)
